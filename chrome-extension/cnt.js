...
function getCurrentVideoInfo() {
  ...
  // 서버 API 실패 시 사용하는 fallback 분석용 정보.
  // YouTube SPA 이동 후 이전 영상 metadata가 남을 수 있으므로 videoId가 맞을 때만 사용한다.
  ...
  const descriptionText = getYoutubeDescriptionText();
  const hashtagKeywords = extractHashtags(descriptionText);
  const titleChannelKeywords = extractKeywords(`${title} ${channelName}`);

  const keywords = [
    ...titleChannelKeywords,
    ...hashtagKeywords,
    ...safeMetadataKeywords.slice(0, 4)
  ];
  ...
  return {
    ...
    keywords: [...new Set([...titleChannelKeywords, ...hashtagKeywords, ...scoreResult.matchedKeywords])].slice(0, 16),
    analysisSource: "dom-fallback"
  };
}
