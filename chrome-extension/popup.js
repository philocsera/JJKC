...
async function fetchServerVideoAnalysis(videoId) {
  if (!videoId) return null;

  try {
    const res = await fetch(`${SITE_URL}/api/extension/analyze-video`, {
      method: "POST",
      headers: {
...
async function analyzeCurrentYoutubePage() {
  ...
  try {
    ...
    const serverVideo = await fetchServerVideoAnalysis(domVideo.videoId);
    const video = serverVideo || domVideo;
    const analysisSourceText = serverVideo
      ? "YouTube API 기반 태그 분석"
      : "브라우저 DOM 기반 fallback 분석";
    ...
    const tagHtml = (video.tags || []).length
      ? video.tags.slice(0, 20).map((k) => `<span class="pill green">${safeText(k)}</span>`).join("")
      : `<span class="muted">YouTube API 태그 없음 또는 fallback 분석</span>`;
    ...
    youtubeAnalysis.innerHTML = `
      ...
      <div class="section-title">YouTube API 태그</div>
      <div>${tagHtml}</div>
    ...
    youtubeAnalysis.innerHTML = `
      ...
      <p class="muted">유튜브 영상 페이지에서 다시 시도하세요. 서버 분석 실패 시 DOM fallback을 사용합니다.</p>
    `;
  }
}
